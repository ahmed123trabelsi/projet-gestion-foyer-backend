...
import tn.esprit.ahmed_4twin7.repository.IChambreRepository;
import tn.esprit.ahmed_4twin7.repository.IEtudiantRepository;
import tn.esprit.ahmed_4twin7.repository.IReservationRepository;
...
//    }
    ...
    public Reservation annulerReservation(long cinEtudiant) {
        ...
        // Find the reservation by the student's cin
        Reservation reservation = reservationRepository.findReservationByEtudiant_Cin(cinEtudiant);

        if (reservation != null) {
            // Update reservation state
            reservation.setEstValide(false);

            // Disassociate the student
            for (Etudiant etudiant : reservation.getEtudiant()) {
                etudiant.getReservations().remove(reservation);
            }
            reservation.setEtudiant(null);

            // Disassociate the room
            ...
            Set<Etudiant> etudiants = reservation.getEtudiant();
            if (etudiants != null) {
                etudiants.removeIf(Objects::isNull); // Remove null elements, if any
    ...
    private Chambre findChambreForReservation(Reservation reservation) {
        for (Chambre chambre : chambreRepository.findAll()) {
            if (chambre.getReservation().contains(reservation)) {
                return chambre;
            }
        }
        return null;
